...
export default function AuditorsPage() {
  ...
  const handleRevokeAccess = async (auditorAddress: string, payrollId: number) => {
    ...
    try {
      ...
      setAuditors(
        auditors.map((record) =>
          record.auditorAddress === auditorAddress && record.payrollId === payrollId
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-2">
          Grant or revoke auditor access to your payroll batches. Auditors can verify payroll
          records without accessing public information.
        </p>
      </div>
      ...
      <Card className="p-6">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-2">Payroll Batch ID</label>
            <Input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium mb-2">Auditor Wallet Address</label>
            <Input
              type="text"
              placeholder="0x..."
      ...
      <Card className="p-6">
        <h2 className="text-xl font-semibold mb-4">Active Auditor Permissions</h2>
        {auditors.length === 0 ? (
              ...
              .map((record) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-gray-600">Payroll ID: {record.payrollId}</p>
                  </div>
                  <Button
                    onClick={() => handleRevokeAccess(record.auditorAddress, record.payrollId)}
                    variant="destructive"
                    size="sm"
                  >
                    Revoke
      ...
      <Card className="p-6 bg-blue-50 border-blue-200">
        <h3 className="font-semibold text-blue-900 mb-2">🔐 Privacy Information</h3>
        <ul className="text-sm text-blue-800 space-y-1">
          <li>• Auditors can view full payroll details only for batches they have access to</li>
