...
export function usePrivacyVisibility(employeeAddress?: string, payrollId?: number) {
  ...
  useEffect(() => {
    ...
    const determinePermissions = async () => {
      try {
        if (employeeAddress && currentUser.toLowerCase() === employeeAddress.toLowerCase()) {
 ...
 */
export function formatSalaryDisplay(salary: number | undefined, canViewSalary: boolean): string {
